...
if [ -z "$PYTHON_BIN" ]; then
  if command -v python >/dev/null 2>&1; then
    ...
    echo "Python not found. Install Python 3 or set PYTHON_BIN to a valid interpreter."
    exit 1
  fi
fi
...
render_prompt() {
  ...
  local run_log="$7"
  local run_meta="$8"
  $PYTHON_BIN - "$src" "$dst" "$PRD_PATH" "$AGENTS_PATH" "$PROGRESS_PATH" "$ROOT_DIR" "$GUARDRAILS_PATH" "$ERRORS_LOG_PATH" "$ACTIVITY_LOG_PATH" "$GUARDRAILS_REF" "$CONTEXT_REF" "$ACTIVITY_CMD" "$NO_COMMIT" "$story_meta" "$story_block" "$run_id" "$iter" "$run_log" "$run_meta" <<'PY'
import sys
...
select_story() {
  local meta_out="$1"
  local block_out="$2"
  $PYTHON_BIN - "$PRD_PATH" "$meta_out" "$block_out" "$STALE_SECONDS" <<'PY'
import json
import os
import sys
from pathlib import Path
...
update_story_status() {
  local story_id="$1"
  local new_status="$2"
  $PYTHON_BIN - "$PRD_PATH" "$story_id" "$new_status" <<'PY'
import json
import os
import sys
from pathlib import Path
...
append_run_summary() {
  local line="$1"
  $PYTHON_BIN - "$ACTIVITY_LOG_PATH" "$line" <<'PY'
import sys
from pathlib import Path
